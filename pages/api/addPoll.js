...
async function addPoll(req, res) {
  ...
  // first reset all poll responses
  users.update({}, { $set: { poll_response: -1 } }, { multi: true });

  // first search for document in MongoDB database that matches the array.
